...
## 4. 配置运行项
    ...
    2. 在Server选项卡：填写配置项， __注意__ 需要在 VM options 中添加 -Djava.library.path="" 命令（否则有可能出现 java.lang.UnsatisfiedLinkError: no jniopenblas in java.library.path 错误）
    3. 在Deployment选项卡 添加 war包，这里有2个war ，必须选择 __AIOnlineRecommedation:war exploded__ Application context默认为/（该属性即为该网站的根目录）
